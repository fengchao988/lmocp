    ...
    public Result<?> getUserPermissionByToken() {
        ...
        try {
            ...
            if (!PermissionDataUtil.hasIndexPage(metaList)) {
                List<SysPermission> menuList = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页"));
                if (CollectionUtils.isNotEmpty(menuList)) {
                    SysPermission indexMenu = menuList.get(0);
