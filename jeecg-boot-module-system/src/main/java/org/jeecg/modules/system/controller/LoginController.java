...
public class LoginController {
    ...
    public Result<?> login(@RequestBody SysLoginModel sysLoginModel) {
        Result<?> checkResult = checkUser(sysLoginModel);
        if (!checkResult.isSuccess()) {
            return checkResult;
        }
        SysUser sysUser = (SysUser) checkResult.getResult();
    ...
    private Result<?> checkUser(SysLoginModel user) {
        ...
        if (!syspassword.equals(userpassword)) {
            return Result.error(io.ttyys.micrc.api.common.dto.CommonConstant.SC_INTERNAL_SERVER_ERROR_500, "用户名或密码错误");
        }
        return Result.OK(sysUser);
    }

    @ApiOperation("登录接口--专为token")
    @PostMapping("/getTokenLogin")
    public String getTokenLogin(@RequestBody SysLoginModel user) {
